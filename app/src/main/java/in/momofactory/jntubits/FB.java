...
public class FB extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(mode.contentEquals(AppSettings.LEARNING)) {
            ...
            fragment = FBLearnFragment.newInstance(true, questionIds, pos);
        } else if(mode.contentEquals(AppSettings.REVISION)) {
            //Load revision fragment
            fragment = FBRevFragment.newInstance(questionIds, pos);
        }

        fragmentTransaction.replace(R.id.fbContainer, fragment);
